...
// Provider streams from the xAI (Grok) API, which is OpenAI-compatible.
type Provider struct {
	Model    string
	Endpoint string
}

// New returns an xAI provider with default settings.
func New() *Provider {
...
func (p *Provider) Stream(ctx context.Context, w http.ResponseWriter, apiKey, prompt string) error {
	...
	if err != nil {
		sseutil.WriteError(w, "xai request failed")
		return fmt.Errorf("xai: do request: %w", err)
	}
	defer func() { _ = resp.Body.Close() }()

	if resp.StatusCode != http.StatusOK {
		sseutil.WriteError(w, fmt.Sprintf("xai returned status %d", resp.StatusCode))
